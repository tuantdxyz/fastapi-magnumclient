...
def config_cluster_kubernetes_magnum(cluster, certs):
    """Return file configuration for the given kubernetes cluster. TuanTD"""
    cfg_k8s = ("apiVersion: v1\n"
               "clusters:\n"
               "- cluster:\n"
               "    certificate-authority-data: %(ca)s\n"
               "    server: %(api_address)s\n"
               ...
               % {'name': cluster.name,
                  'api_address': cluster.api_address,
                  'key': base64.encode_as_text(certs['private_key']),
                  'cert': base64.encode_as_text(certs['public_key']),
                  'ca': base64.encode_as_text(certs['ca'])})
